...
{                             // vec_E is a vector of primitives of dimension Dim (Dim=3 if using cloud_util)
  vec_E<Primitive<Dim>> prs;  // This is what is going to be returned in this function
  if (ss_ptr_->best_child_.empty())
    return prs;

  std::unordered_map<Key, bool> added;  // added is a c++ map. A c++ map is like an array, but it's indexed with the
                                        // Key. This map will contain bool variables
  ...
  while (!q.empty())
  {
    ...
    for (int i = 0; i < size; i++)
    {
      ...
      q.pop();
      // pred_hashkey is a vector that contains the keys of all the precessors of a node.
      for (unsigned int j = 0; j < currNode_ptr->pred_hashkey.size(); j++)
      {
        Key pred_key =
            currNode_ptr->pred_hashkey[j];  // pred_key is the key of one of the predecesors of the current node
        Key key_pair = currNode_ptr->hashkey + pred_key;

        // added.count(XX) counts the number of elements with a key equivalent to XX
        if (added.count(key_pair) == 1 ||
            std::isinf(currNode_ptr->pred_action_cost[j]))  // skip the pred if the cost is inf
          continue;
