...
public:
  ...
  decimal_t cal_heur(const Waypoint<Dim>& state, const Waypoint<Dim>& goal) const
  {
    ...
    if (state.use_pos && state.use_vel && state.use_acc && !state.use_jrk && goal.use_pos && goal.use_vel &&
        goal.use_acc && !goal.use_jrk)
    {
      ...
      for (auto t : ts)
      {
        if (t < t_bar)
          continue;
        decimal_t cost =
            a * t - c / t - d / 2 / t / t - e / 3 / t / t / t - f / 4 / t / t / t / t - g / 5 / t / t / t / t / t;
        if (cost < min_cost)
          min_cost = cost;
      }
      return min_cost;
    ...
    else if (state.use_pos && state.use_vel && state.use_acc && !state.use_jrk && goal.use_pos && goal.use_vel &&
             !goal.use_acc && !goal.use_jrk)
    {
      const Vecf<Dim> dp = goal.pos - state.pos;
      const Vecf<Dim> v0 = state.vel;
   ...
   */
  virtual void get_succ(const Waypoint<Dim>& curr, vec_E<Waypoint<Dim>>& succ, std::vector<Key>& succ_idx,
